          ...
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            <nav className="hidden lg:flex items-center gap-1 bg-background/50 rounded-xl p-1 border border-border/50">
              ...
              <div ref={dropdownRef} className="relative">
                ...
                {/* Dropdown Menu */}
                {gamesDropdownOpen && (
                  <div className="absolute top-full left-0 mt-2 w-80 rounded-2xl overflow-hidden z-50 border border-white/10 bg-[#080a0f] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.9)] animate-in fade-in slide-in-from-top-2 duration-200">
                    {/* Glow effect */}
                    ...
                    <div className="relative p-3">
                      ...
                      {GAMES.filter(g => g.status === 'live').map((game) => (
                        <Link
                          ...
                          className={`group flex items-center gap-4 p-3 rounded-xl transition-all duration-200 ${isActive(game.href) ? 'bg-accent/20 ring-1 ring-accent/50' : 'hover:bg-white/5'}`}
                        >
                          <div className="relative">
                            <div className="absolute inset-0 bg-accent/30 blur-xl rounded-xl opacity-0 group-hover:opacity-100 transition-opacity" />
                            <div className="relative w-12 h-12 rounded-xl bg-gradient-to-br from-accent/20 to-accent/5 border border-accent/20 flex items-center justify-center group-hover:border-accent/40 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">{game.description}</div>
                          </div>
                          <div className="flex-shrink-0">
                            <span className="px-2 py-1 text-[10px] font-bold bg-green-500/20 text-green-400 rounded-lg border border-green-500/30">PLAY</span>
                          </div>
                        </Link>
                      ))}
                    </div>
                    ...
                    <div className="relative p-3 bg-black/20">
                      ...
                      {GAMES.filter(g => g.status === 'soon').map((game) => (
                        <div
                          key={game.label}
                          className="flex items-center gap-4 p-3 rounded-xl opacity-50"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-bold text-gray-500">{game.label}</div>
                            <div className="text-sm text-gray-600">{game.description}</div>
                          </div>
                          <div className="flex-shrink-0">
                            <span className="px-2 py-1 text-[10px] font-bold bg-white/5 text-gray-600 rounded-lg border border-white/5">SOON</span>
                          </div>
                        </div>
                      ))}
                    </div>
