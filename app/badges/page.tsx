      ...
      <main className="flex-1 py-4 sm:py-8 md:py-12 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-6 sm:mb-10">
            <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-accent/10 border border-accent/20 mb-3 sm:mb-4">
              <Award className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-accent" />
              <span className="text-[10px] sm:text-xs font-semibold text-accent uppercase tracking-wide">Achievements</span>
            </div>
            ...
            <>
              ...
              {account?.address && stats && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-6 sm:mb-10">
                  <div className="p-3 sm:p-4 rounded-xl bg-accent/10 border border-accent/20">
                    <div className="flex items-center gap-1.5 sm:gap-2 text-accent mb-1">
                      <Trophy className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                      <span className="text-[10px] sm:text-xs uppercase tracking-wider font-medium">Earned</span>
                    </div>
                    <div className="text-xl sm:text-2xl font-bold text-white">
                      {unlockedCount} <span className="text-xs sm:text-sm text-text-muted">/ {totalCount}</span>
                    </div>
                  </div>
                  ...
                  {(Object.entries(stats.badgesByTier) as [BadgeTier, number][])
                    ...
                    .slice(0, 3)
                    .map(([tier, count]) => (
                      <div key={tier} className="p-3 sm:p-4 rounded-xl bg-white/[0.03] border border-white/[0.08]">
                        <div className="text-[10px] sm:text-xs uppercase tracking-wider font-medium text-text-muted mb-1 capitalize">
                          {tier}
                        </div>
              ...
              {account?.address && nextBadges.length > 0 && (
                <div className="mb-6 sm:mb-10">
                  <div className="flex items-center gap-2 mb-3 sm:mb-4">
                    ...
                    <h2 className="text-base sm:text-lg font-semibold text-white">Almost There</h2>
                  </div>
              ...
              {/* NFT Teaser */}
              <div className="mt-8 sm:mt-12 p-4 sm:p-6 rounded-2xl bg-gradient-to-br from-purple-500/10 to-accent/10 border border-purple-500/20 text-center">
                <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">🎨</div>
                <h3 className="text-base sm:text-lg font-bold text-white mb-1.5 sm:mb-2">Badge NFTs Coming Soon</h3>
