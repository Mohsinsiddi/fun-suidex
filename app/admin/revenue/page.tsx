    ...
    <div className="min-h-screen flex bg-background">
      ...
      <aside className="w-64 admin-sidebar flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <Link href="/admin/revenue" className="admin-nav-item active"><DollarSign className="w-5 h-5" />Revenue</Link>
                ...
                <div className="card p-6">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className={`text-2xl font-bold mt-1 ${data.pendingPayments > 0 ? 'text-warning' : ''}`}>{data.pendingPayments || 0}</p>
                      <p className="text-sm text-text-secondary mt-1">awaiting approval</p>
                    </div>
                    <div className={`p-3 rounded-lg ${data.pendingPayments > 0 ? 'bg-warning/10 text-warning' : 'bg-card text-text-secondary'}`}><RefreshCw className="w-5 h-5" /></div>
                  </div>
                </div>
              </div>
